...
namespace GuardianLamppost.LoadingReader.Common.Entitites {
    public class ThreadListThread {
        ...
        public ThreadListThread(HtmlNode threadNode) {
            StatusIcon = threadNode.ParentNode.ParentNode.ChildNodes.Single(x => x.Attributes["class"]?.Value == "forum_forum_tbr_td thread_status").FirstChild.Attributes["src"]?.Value;
